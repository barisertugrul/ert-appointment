...
final class AppointmentApiController
{
    ...
    public function create(WP_REST_Request $request): WP_REST_Response|WP_Error
    {
        ...
        $required = ['provider_id', 'start_datetime', 'customer_name', 'customer_email'];
        ...
        try {
            ...
            return new WP_Error('erta_slot_unavailable', $e->getMessage(), ['status' => 409]);
        } catch (\Throwable $e) {
            return new WP_Error('erta_booking_error', $e->getMessage(), ['status' => 500]);
        }
    ...
    public function get(WP_REST_Request $request): WP_REST_Response|WP_Error
    {
        ...
        if ($appointment === null) {
            return new WP_Error('erta_not_found', __('Appointment not found.', 'ert-appointment'), ['status' => 404]);
        }
    ...
    public function confirm(WP_REST_Request $request): WP_REST_Response|WP_Error
    {
        try {
            ...
            return new WP_Error('erta_confirm_error', $e->getMessage(), ['status' => 400]);
        }
    }
    ...
    public function cancel(WP_REST_Request $request): WP_REST_Response|WP_Error
    {
        ...
        try {
            ...
            return new WP_Error('erta_cancel_error', $e->getMessage(), ['status' => 400]);
        }
    }
    ...
    public function reschedule(WP_REST_Request $request): WP_REST_Response|WP_Error
    {
        ...
        try {
            ...
            return new WP_Error('erta_reschedule_error', $e->getMessage(), ['status' => 400]);
        }
    }
    ...
    public function myAppointments(WP_REST_Request $request): WP_REST_Response
    {
        ...
        return new WP_REST_Response([
            'items' => array_map([$this, 'formatAppointment'], $items),
        ]);
    }
    ...
    public function adminList(WP_REST_Request $request): WP_REST_Response
    {
        ...
        return new WP_REST_Response([
            'items'      => array_map([$this, 'formatAppointment'], $result['items']),
            'total'      => $result['total'],
            'page'       => $page,
            'per_page'   => $perPage,
    ...
    public function providerList(WP_REST_Request $request): WP_REST_Response
    {
        ...
        $filters = ['provider_id' => $providerIds[0]]; // simplified — multi-provider handled below
        ...
        if (count($providerIds) === 1) {
            ...
            foreach ($providerIds as $pid) {
                $r = $this->repository->paginate($page, $perPage, ['provider_id' => $pid]);
