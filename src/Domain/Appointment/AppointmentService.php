...
final class AppointmentService
{
        ...
        if (! $appointment->isCancellable()) {
            throw new RuntimeException(
                sprintf(
                    ...
                    __('Cannot cancel an appointment with status "%s".', 'ert-appointment'),
    ...
    public function reschedule(int $appointmentId, RescheduleDTO $dto, int $actorUserId): Appointment
    {
        ...
        if (! $original->isReschedulable()) {
            throw new RuntimeException(
                __('This appointment cannot be rescheduled.', 'ert-appointment')
            );
        }
    ...
    private function assertSlotAvailable(
        ...
        throw new SlotNotAvailableException(
            __('The selected time slot is no longer available. Please choose another.', 'ert-appointment')
        );
    }
    ...
    private function assertCanActOn(Appointment $appointment, int $actorUserId, string $action): void
    {
        ...
        global $wpdb;
        $isProvider = $wpdb->get_var($wpdb->prepare(
        ...
        throw new RuntimeException(
            sprintf(
                ...
                __('You do not have permission to %s this appointment.', 'ert-appointment'),
                $action
            )
        );
    }
