param(
    ...
    [string]$Tester = "",
    [string]$Environment = "",
    [string]$TemplateFile = ".\docs\pro-metadata-transition-test-report.md"
)

$ErrorActionPreference = 'Stop'

$manualScript = ".\scripts\pro-metadata-transition-manual-test.ps1"
...
$jsonOutput = Join-Path $OutputDir "pro-metadata-transition-$timestamp.json"
$mdOutput = Join-Path $OutputDir "pro-metadata-transition-report-$timestamp.md"

Write-Host "[1/2] Pro metadata transition manual test başlatılıyor..." -ForegroundColor Cyan
